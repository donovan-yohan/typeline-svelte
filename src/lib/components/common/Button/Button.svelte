...
{#if $$slots.default}
	<span class={'inline-block text-lg ' + $$props.class}>
		<button
			...
			class={cx({
				// base style
				'relative z-10 whitespace-nowrap py-0.5 px-1 font-bold uppercase tracking-[0.25rem] transition-all duration-300 focus-visible:outline-none': true,
				...
				'before:hover:w-[98%] before:focus-visible:w-[98%]': true,
				// after style
				...
				'after:hover:left-[95%] after:hover:opacity-100 after:focus-visible:left-[95%] after:focus-visible:opacity-100': true
			})}><slot /></button
		>
	</span>
{/if}

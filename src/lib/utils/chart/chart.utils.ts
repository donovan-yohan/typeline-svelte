import type { ChartStatType, StatType } from './chart.utils.definition';
...
const weightedAverage = (array: number[], i: number, totalSamples: number, weight: number) => {
	let value = 0;
...
export const generateChartStats = (stats: StatType[]): ChartStatType[] => {
	...
	return stats.map((stat, i) => {
		const correctToTime = stats.slice(0, i + 1).reduce((acc, stat) => acc + stat.correct, 0);
		const incorrectToTime = stats.slice(0, i + 1).reduce((acc, stat) => acc + stat.incorrect, 0);
		const correctedToTime = stats.slice(0, i + 1).reduce((acc, stat) => acc + stat.corrected, 0);
		...
		return {
			...
			raw: calculateRawWPM(weightedCorrect, 0, 1),
